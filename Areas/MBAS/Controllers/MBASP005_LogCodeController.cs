...
namespace mvcadmin10.Areas.Mis.Controllers
{
    ...
    /// </summary>
    [Area("MBAS")]
    public class MBASP005_LogCodeController : BaseAdminController
    {
        /// <summary>
        /// 控制器建構子
        /// </summary>
        ...
        public MBASP005_LogCodeController(IConfiguration configuration, dbEntities entities)
        {
            db = entities;
        ...
        public ActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            using var sqlData = new z_sqlLogCodes();
        ...
        [Login(RoleList = "Mis")]
        [Security(Mode = enSecurityMode.AddEdit)]
        public IActionResult CreateEdit(LogCodes model)
        {
            //檢查是否有違反 Metadata 中的 Validation 驗證
            if (!ModelState.IsValid) return View(model);
            //檢查是否重覆輸入值
            using var dpr = new DapperRepository();
            if (dpr.IsDuplicated(model, "CodeNo"))
            {
        ...
        {
            using var sqlData = new z_sqlLogCodes(); return sqlData.Delete(id);
        }
    }
}