...
namespace mvcadmin10.Areas.Mis.Controllers
{
    ...
    /// </summary>
    [Area("MBAS")]
    public class MBASP002_LanguageController : BaseAdminController
    {
        /// <summary>
        /// 控制器建構子
        /// </summary>
        ...
        public MBASP002_LanguageController(IConfiguration configuration, dbEntities entities)
        {
            db = entities;
        ...
        public ActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            using var sqlData = new z_sqlLanguages();
        ...
        [Login(RoleList = "Mis")]
        [Security(Mode = enSecurityMode.AddEdit)]
        public IActionResult CreateEdit(Languages model)
        {
            ...
            if (dpr.IsDuplicated(model, "LangNo"))
            {
                ModelState.AddModelError("LangNo", "編號重覆輸入!!");
                return View(model);
            }
            //執行新增或修改資料
            using var sqlData = new z_sqlLanguages();
        ...
        {
            using var sqlData = new z_sqlLanguages(); return sqlData.Delete(id);
        }
    }
}