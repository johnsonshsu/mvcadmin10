...
namespace mvcadmin10.Areas.Mis.Controllers
{
    ...
    /// </summary>
    [Area("MBAS")]
    public class MBASP001_RoleController : BaseAdminController
    {
        /// <summary>
        /// 控制器建構子
        /// </summary>
        ...
        public MBASP001_RoleController(IConfiguration configuration, dbEntities entities)
        {
            db = entities;
        ...
        public IActionResult CreateEdit(Roles model)
        {
            ...
            if (dpr.IsDuplicated(model, "RoleNo"))
            {
                ModelState.AddModelError("RoleNo", "編號重覆輸入!!");
                return View(model);
            }
            //執行新增或修改資料
            using var sqlData = new z_sqlRoles();
        ...
        {
            using var sqlData = new z_sqlRoles(); return sqlData.Delete(id);
        }
    }
}