...
namespace mvcadmin10.Areas.Mis.Controllers
{
    ...
    /// </summary>
    [Area("USET")]
    public class USETP003_CityController : BaseAdminController
    {
        /// <summary>
        /// 控制器建構子
        /// </summary>
        ...
        public USETP003_CityController(IConfiguration configuration, dbEntities entities)
        {
            db = entities;
        ...
        public ActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            using var sqlData = new z_sqlCitys();
        ...
        [Login(RoleList = "User")]
        [Security(Mode = enSecurityMode.AddEdit)]
        public IActionResult CreateEdit(Citys model)
        {
            //檢查是否有違反 Metadata 中的 Validation 驗證
            if (!ModelState.IsValid) return View(model);
            //檢查是否重覆輸入值
            using var dpr = new DapperRepository();
            if (dpr.IsDuplicated(model, "CityNo"))
            {
                ModelState.AddModelError("CityNo", "編號重覆輸入!!");
                return View(model);
            }
            //執行新增或修改資料
            using var sqlData = new z_sqlCitys();
        ...
        {
            using var sqlData = new z_sqlCitys(); return sqlData.Delete(id);
        }
    }
}