...
namespace mvcadmin10.Areas.Mis.Controllers
{
    ...
    /// </summary>
    [Area("USET")]
    public class USETP002_CodeDataController : BaseAdminController
    {
        /// <summary>
        /// 控制器建構子
        /// </summary>
        ...
        public USETP002_CodeDataController(IConfiguration configuration, dbEntities entities)
        {
            db = entities;
        ...
        public IActionResult Init(string id = "", int initPage = 1)
        {
            ...
            {
                var sqlData = new z_sqlCodeBases();
                SessionService.BaseNo = sqlData.GetDataList()
                .Where(m => m.IsAdmin == false)
                .OrderBy(m => m.BaseNo)
                .FirstOrDefault().BaseNo;
            }
            //這裏可以寫入初始程式
        ...
        public ActionResult Index(int id = 1)
        {
            ...
            SetIndexViewBag();
            //設定主檔資料列表
            using var sqlCodeBases = new z_sqlCodeBases();
            var masterList = sqlCodeBases.GetDropDownList(false, true);
        ...
        public IActionResult CreateEdit(int id = 0)
        {
            ...
            if (id == 0)
            {
                model.BaseNo = SessionService.BaseNo;
        ...
        [Login(RoleList = "User")]
        [Security(Mode = enSecurityMode.AddEdit)]
        public IActionResult CreateEdit(CodeDatas model)
        {
            //檢查是否有違反 Metadata 中的 Validation 驗證
            if (!ModelState.IsValid) return View(model);
            //檢查是否重覆輸入值
            using var dpr = new DapperRepository();
            string sql_check = $" BaseNo = '{SessionService.BaseNo}' AND CodeNo = '{model.CodeNo}' ";
            if (dpr.IsDuplicated(model, "CodeNo", sql_check))
            {
                ModelState.AddModelError("CodeNo", "編號重覆輸入!!");
                return View(model);
            }
            //執行新增或修改資料
            model.BaseNo = SessionService.BaseNo;
        ...
        {
            using var sqlData = new z_sqlCodeDatas(); return sqlData.Delete(id);
        }
    }
}