...
namespace mvcadmin10.Areas.Mis.Controllers
{
    ...
    /// </summary>
    [Area("UHOM")]
    public class UHOMP006_PriceController : BaseAdminController
    {
        /// <summary>
        /// 控制器建構子
        /// </summary>
        ...
        public UHOMP006_PriceController(IConfiguration configuration, dbEntities entities)
        {
            db = entities;
        ...
        public ActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            using var sqlData = new z_sqlPricings();
        ...
        [Login(RoleList = "User")]
        [Security(Mode = enSecurityMode.AddEdit)]
        public IActionResult CreateEdit(Pricings model)
        {
        ...
        {
            using var sqlData = new z_sqlPricings(); return sqlData.Delete(id);
        }
        ...
        {
            var sqlData = new z_sqlPricings();
            var model = sqlData.GetData(id);
            sqlData.SetParent(id, SessionService.Page);
            ActionService.SetReturnUrl(ActionService.Area, ActionService.Controller, ActionService.Init, "", SessionService.Page);
            return RedirectToAction(ActionService.Init, "UHOMP006A_PricingDetail", new { area = ActionService.Area, id = model.PricingNo, page = 1 });
        }
    }
}