...
namespace mvcadmin10.Areas.Mis.Controllers
{
    ...
    /// </summary>
    [Area("UHOM")]
    public class UHOMP006A_PricingDetailController : BaseAdminController
    {
        /// <summary>
        /// 控制器建構子
        /// </summary>
        ...
        public UHOMP006A_PricingDetailController(IConfiguration configuration, dbEntities entities)
        {
            db = entities;
        ...
        public ActionResult Index(int id = 1)
        {
            ...
            string subActionName = $"{SessionService.ParentNo} {SessionService.ParentName}";
            ActionService.SetActionName(enAction.Index);
            ActionService.SetSubActionName(subActionName);
            ActionService.SetActionCardSize(enCardSize.Max);
            //取得資料列表集合
            using var sqlData = new z_sqlPricingDetails();
        ...
        public IActionResult CreateEdit(int id = 0)
        {
            ...
            if (id == 0)
            {
                model.PricingNo = SessionService.BaseNo;
        ...
        [Login(RoleList = "User")]
        [Security(Mode = enSecurityMode.AddEdit)]
        public IActionResult CreateEdit(PricingDetails model)
        {
        ...
        {
            using var sqlData = new z_sqlPricingDetails(); return sqlData.Delete(id);
        }
    }
}