...
namespace mvcadmin10.Areas.Mis.Controllers
{
    ...
    /// </summary>
    [Area("UORG")]
    public class UORGP003_TitleController : BaseAdminController
    {
        /// <summary>
        /// 控制器建構子
        /// </summary>
        ...
        public UORGP003_TitleController(IConfiguration configuration, dbEntities entities)
        {
            db = entities;
        ...
        public ActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            using var sqlData = new z_sqlTitles();
        ...
        public IActionResult CreateEdit(Titles model)
        {
            ...
            if (dpr.IsDuplicated(model, "TitleNo"))
            {
                ModelState.AddModelError("TitleNo", "編號重覆輸入!!");
                return View(model);
            }
            //執行新增或修改資料
            using var sqlData = new z_sqlTitles();
        ...
        {
            using var sqlData = new z_sqlTitles(); return sqlData.Delete(id);
        }
    }
}